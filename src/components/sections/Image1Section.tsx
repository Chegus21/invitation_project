...
export const Image1Section: React.FC<SectionProps> = ({ data }) => {
  const caps = getCapabilities(data);

  const bgImage = data.customization?.image1
    ? data.customization.image1.startsWith("http")
      ? `url("${data.customization.image1}")`
      : `url("${import.meta.env.BASE_URL}${data.customization.image1.replace(/^\//, "")}")`
    : undefined;

  // 👇 IMPORTANTE: evitar render si no hay imagen
  if (!bgImage) {
    return null;
  }
      ...
      >
        ...
        >
          {data.customization?.text1 && (
            <p className="text-white text-lg sm:text-xl md:text-2xl lg:text-3xl montserrat-custom leading-snug [text-shadow:_0_2px_4px_rgb(0_0_0_/_70%)]">
              {data.customization.text1}
            </p>
          )}
        </motion.div>
